...
names(GSEC15c_2010)[names(GSEC15c_2010) == "hh"] <- "id_hh"
...
non_durable_2010 <- filter(GSEC15c_2010, id_hh %in% final_vector_sample$id_hh) %>% arrange(id_hh)

merged <- merge(matched_hh_2010, non_durable_2010, by = "id_hh")  ###### THIS DATASET IS FINAL

final_dataset10_non_durable <- as.data.frame(merged$id_hh)
names(final_dataset10_non_durable)[names(final_dataset10_non_durable) == "merged$id_hh"] <- "case_id"



final_dataset10_non_durable <- summarise(final_dataset10_non_durable,
...
nrow(as.data.frame(unique(final_dataset10_non_durable$case_id))) # final_dataset10_non_durable has 2157 households. OK!


rm(non_durable_2010, final_vector_sample, GSEC15c_2010, matched_hh_2010, hh_2010, merged)

final_dataset10_non_durable$hh_g04a <- 1
final_dataset10_non_durable$hh_g06a <- 1
final_dataset10_non_durable$hh_g07a <- 1

####################
colSums(is.na(final_dataset10_non_durable))
filter(final_dataset10_non_durable, is.na(hh_g04a) == F & hh_g04a != 0)

##################
# a)        ###### How many and which items are there?
##################

final_dataset10_non_durable %>% 
  summarise(item_list = unique(hh_g02)) %>% arrange() # 38 items
...
as.data.frame(final_dataset10_non_durable %>%                          
...
price_list10_non_durable <- final_dataset10_non_durable %>% 
  group_by(hh_g02) %>% 
...
final_dataset10_non_durable <- right_join(final_dataset10_non_durable, price_list11_non_durable, by = "hh_g02") # replace price_list11_non_durable, with price_list10_non_durable
...
b1_non_durables10 <- final_dataset10_non_durable %>%                                 
...
nrow(as.data.frame(unique(b1_non_durables10$case_id))) # 2157 OK!

# total weekly consumption in sample
sum(b1_non_durables10$total_consumption_weekly)




# save household level non-durbable consumption in 2010 wave
write_csv(b1_non_durables10, "/Users/andreasalem/R_oba/Thesis/Data_clean/items_2010_non-durab.csv") 
