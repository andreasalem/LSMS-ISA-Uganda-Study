...
source("/Users/andreasalem/R_oba/Thesis/do_files/do_items_food_2010") # we need just final_dataset11_food
...
sum(b1_food11$total_consumption_food) # OK!
sum(graph_food_categories10$rural_share,na.rm =T) + sum(graph_food_categories10$urban_share, na.rm =T) # NOT OK
...
# order by shares the pie chart I should
sort(graph_food_categories$rural_share)
...
# food production
graph_quantiles_food <- result_2010 %>% group_by(category_quantile_food) %>% 
  summarise(tot_food = sum(total_consumption_food, na.rm = T),
...
graph_quantiles_food <- gather(graph_quantiles_food, category, total, tot_food:'gits/in-kind sources', factor_key = T) %>% 
  mutate(total_perc = case_when(category_quantile_food == 1 ~ total*100/17713983, # as.numeric(graph_quantiles_food[1,3])
                                category_quantile_food == 2 ~ total*100/27345476,
                                category_quantile_food == 3 ~ total*100/37158224,
                                category_quantile_food == 4 ~ total*100/98718374,
                                category_quantile_food == 5 ~ total*100/3598896))

# plot 1
ggplot(graph_quantiles_food[6:20,], aes( x = category_quantile_food, y = total_perc, fill = category)) + 
  geom_bar( stat = "identity", position = "stack" ) +
  coord_flip() +
  scale_fill_brewer( palette = "YlGnBu" ) +
  theme_minimal() + theme( legend.position = "bottom" )
...
ggplot(data=filter(b1_food11, urban == 0), aes(x=purchases)) +
  geom_density(adjust=1.5, alpha=.4) +
  theme_ipsum() +
  xlim(c(0, 800000))
...
vec <- as.vector(TOT_consumption$wgt10)
