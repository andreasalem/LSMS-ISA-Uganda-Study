...
names(GSEC15C_2011)[names(GSEC15C_2011) == "HHID"] <- "id_hh"
...
non_durable_2011 <- filter(GSEC15C_2011, id_hh %in% final_vector_sample$id_hh) %>% arrange(id_hh)

merged <- merge(matched_hh_2011, non_durable_2011, by = "id_hh")  ###### THIS DATASET IS FINAL

final_dataset11_non_durable <- as.data.frame(merged$id_hh)
names(final_dataset11_non_durable)[names(final_dataset11_non_durable) == "merged$id_hh"] <- "case_id"



final_dataset11_non_durable <- summarise(final_dataset11_non_durable,
...
nrow(as.data.frame(unique(final_dataset11_non_durable$case_id))) # 2157 households


rm(non_durable_2011, final_vector_sample, GSEC15C_2011, matched_hh_2011, hh_2011, merged)

final_dataset11_non_durable$hh_g04a <- 1
final_dataset11_non_durable$hh_g06a <- 1
final_dataset11_non_durable$hh_g07a <- 1

####################
colSums(is.na(final_dataset11_non_durable))
...
final_dataset11_non_durable %>% 
  summarise(item = unique(hh_g02)) %>% arrange(item) # 38 items
...
as.data.frame(final_dataset11_non_durable %>%                          
...
price_list11_non_durable <- final_dataset11_non_durable %>% 
  group_by(hh_g02) %>% 
...
final_dataset11_non_durable <- right_join(final_dataset11_non_durable, price_list11_non_durable, by = "hh_g02")
...
final_dataset11_non_durable <- final_dataset11_non_durable %>% 
  rowwise() %>% 
...
# for quantities, we should use sum of 4 variables:  hh_g04a, hh_g05a, hh_g06a, hh_g07a. But for durables, we don't distinguish



# alternative b1, where each item is consumer once and we can't diistinguih the origin of the item
b1_non_durables11 <- final_dataset11_non_durable %>%                                 
...
nrow(as.data.frame(unique(b1_non_durables11$case_id))) # 2157 

# total weekly consumption in sample
sum(b1_non_durables11$total_consumption_weekly)


# save household level non-durbable consumption in 2011 wave
write_csv(b1_non_durables11, "/Users/andreasalem/R_oba/Thesis/Data_clean/items_2011_non-durab.csv") 

# save price list to apply it to other years' non-durables
write_csv(price_list11_non_durable, "/Users/andreasalem/R_oba/Thesis/Data_clean/price_list11_non_durable.csv")  
